# GrowMark

<details>
<summary>Table of contents</summary>
<ol>
  <li><a href="#about-the-project" alt="About the project">about-the-project</a></li>
  <li><a href="#built-with" alt="Built With" >built-with</a></li>
  <li><a href="#authors" alt="Authors" >authors</a></li>
  <li><a href="#license" alt="License" >license</a></li>
  <li><a href="#contact" alt="Contact" >contact</a></li>
</ol>
</details>

## About the project

This project consists of practicing designing a website template using React and TypeScript. The project should be adaptable to all screens and include animations to add a touch of dynamism and interactivity. I will use libraries such as GSAP to create fluid transitions and attractive visual effects. I will also focus on optimizing performance by using coding best practices and optimizing images.

[![live demo link](https://img.shields.io/badge/Live%20Demo-00B9ae?style=for-the-badge)](https://idembele70.github.io/growmark/)

[![Template information](./public/assets/design/app.png)](https://demo.htmlcodex.com/2665/digital-marketing-html-template/)

## Built With

[![React](https://img.shields.io/badge/React-20232A?style=for-the-badge&logo=react&logoColor=61DAFB)](https://fr.reactjs.org/)&nbsp;&nbsp;
[![Styled-Components](https://img.shields.io/badge/styled--components-20232A?style=for-the-badge&logo=styled-components)](https://styled-components.com/)&nbsp;&nbsp;
[![Typescript](https://img.shields.io/badge/typescript-20232A?style=for-the-badge&logo=typescript&logoColor=23007ACC)](https://www.typescriptlang.org/)&nbsp;&nbsp;
[![NPM](https://img.shields.io/badge/npm-20232A?style=for-the-badge&logo=npm&logoColor=23CB3837.)](https://www.npmjs.com/)&nbsp;&nbsp;
[![React Slick](https://img.shields.io/badge/React_Slick-20232A?style=for-the-badge&logo=ReactSlick&logoColor=23CB3837.)](https://www.npmjs.com/)&nbsp;&nbsp;
[![React Countup](https://img.shields.io/badge/React_countup-20232A?style=for-the-badge&logo=Reactcountup&logoColor=23CB3837.)](https://github.com/glennreyes/react-countup)&nbsp;&nbsp;
[![React-Router](https://img.shields.io/badge/React_Router-20232A.svg?style=for-the-badge&logo=react-router&logoColor=CA4245)](https://github.com/)&nbsp;&nbsp;
[![Font Awesome](https://img.shields.io/badge/Font_Awesome-20232A.svg?style=for-the-badge&logo=data:image/png;base64,)](https://github.com/)&nbsp;&nbsp;
[![Github](https://img.shields.io/badge/github-20232A.svg?style=for-the-badge&logo=github&logoColor=white)](https://github.com/)&nbsp;&nbsp;
[![ChatGPT](https://img.shields.io/badge/ChatGPT-20232A.svg?style=for-the-badge&logo=data:image/png;base64,)](https://openai.com/product/gpt-4)&nbsp;&nbsp;

## Authors

- [HTML Codex](https://htmlcodex.com/contact/) created the template [Template](https://htmlcodex.com/digital-marketing-html-template/).
- To develop the application, I used React with Typescript and several NPM libraries.

## License

[![License](https://img.shields.io/badge/license-CC_By_4.0-20232A?style=for-the-badge&logoColor=F7BA3E)](https://htmlcodex.com/license/)&nbsp;&nbsp;

## Contact

[![outlook](https://img.shields.io/badge/Microsoft_Outlook-20232A?style=for-the-badge&logo=microsoft-outlook)](mailto:ibrahimk.dembele@outlook.fr)&nbsp;&nbsp;
[![twitter](https://img.shields.io/badge/twitter-20232A?style=for-the-badge&logo=twitter)](https://twitter.com/Playmaker26_7)&nbsp;&nbsp;
[![github](https://img.shields.io/badge/github-20232A?style=for-the-badge&logo=github)](https://github.com/idembele70)&nbsp;&nbsp;

## Roadmap

- [x] Responsive
  - [x] Desktop
  - [x] Tablet
  - [x] Mobile
- [x] Animation
  - [x] Scroll Trigger
  - [x] Loader
- [ ] Deploy
- [x] README
